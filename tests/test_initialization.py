...
from bhe.initialization import (
    initialize_circular_orbit_ring,
...
class TestCompleteInitialization:
    ...
    def test_initialize_from_baseline_config(self):
        ...
        # Check debris masses are equal (unified system: debris particles start after BHs)
        debris_start_idx = state.n_bh
        debris_masses = state.masses[debris_start_idx:debris_start_idx + state.n_debris]
        assert np.all(debris_masses == params.debris_mass_per_particle)

        # Check no particles accreted yet
        assert state.n_accreted == 0
        assert state.n_active == state.n_total

    def test_deterministic_with_seed(self):
        """Same seed should produce identical initial conditions (unified particle system)."""
        config_path = Path(__file__).parent.parent / 'configs' / 'baseline_config.yaml'
        params = SimulationParameters.from_yaml(str(config_path))
        ...
        # All particle positions and velocities should be identical
        assert np.allclose(state1.positions, state2.positions)
        assert np.allclose(state1.velocities, state2.velocities)

    def test_different_seeds_produce_different_results(self):
        """Different seeds should produce different initial conditions (unified particle system)."""
        config_path = Path(__file__).parent.parent / 'configs' / 'baseline_config.yaml'
        params = SimulationParameters.from_yaml(str(config_path))
